...
<style>
    ...
    /* ---- Export box ---- */
    .export-row { display: flex; gap: 0.75rem; flex-wrap: wrap; margin-top: 0.3rem; }
</style>
...
<div class="hero-wrap">
    ...
    <div class="hero-subtext">
        ...
        Filter by NAICS, state, value, keywords, set-aside, notice type, and date ranges
...
SET_ASIDE_OPTIONS = [
    ...
    "Buy Indian / Indian Small Business",
    "Local Area Set-Aside",
]

# Maps each user-friendly label → lowercase substrings to search for in Set-Aside field
SET_ASIDE_KEYWORD_MAP = {
    "Any Set-Aside (has a set-aside)": [],  # handled specially — any non-empty set-aside
    "8(a)": ["8(a)", "8a"],
    "8(a) Sole Source": ["8(a) sole source", "8a sole source", "sole source 8"],
    "8(a) Competitive": ["8(a) competitive", "8a competitive", "competitive 8(a)", "competitive 8a"],
    "WOSB – Women-Owned Small Business": ["wosb", "women-owned small business", "woman-owned small business", "women owned small"],
    "EDWOSB – Economically Disadvantaged WOSB": ["edwosb", "economically disadvantaged women"],
    "SDVOSB – Service-Disabled Veteran-Owned": ["sdvosb", "service-disabled veteran", "service disabled veteran"],
    "VOSB – Veteran-Owned Small Business": ["vosb", "veteran-owned small business", "veteran owned small"],
    ...
    "Small Disadvantaged Business": ["small disadvantaged", "sdb"],
    "Buy Indian / Indian Small Business": ["buy indian", "indian small business economic", "isbee"],
    "Local Area Set-Aside": ["local area set-aside", "local area"],
}
...
def parse_datetime_series_mixed(series: pd.Series) -> pd.Series:
    """Safely parse a datetime Series that may contain mixed formats or timezones."""
    if series is None:
...
def filter_by_setaside(df: pd.DataFrame, selected_setasides: list) -> pd.DataFrame:
    """Filter dataframe rows by selected set-aside categories."""
    if not selected_setasides or df is None or df.empty:
        return df
    if "Set-Aside" not in df.columns:
        return df

    sa_col = df["Set-Aside"].fillna("").astype(str).str.lower().str.strip()

    # Special case: any non-empty set-aside
    if "Any Set-Aside (has a set-aside)" in selected_setasides:
        mask = sa_col.ne("") & sa_col.ne("—") & sa_col.ne("nan") & sa_col.ne("none")
        return df[mask]

    # Match any row whose Set-Aside contains at least one pattern from any selected category
    combined_mask = pd.Series([False] * len(df), index=df.index)
    for category in selected_setasides:
        patterns = SET_ASIDE_KEYWORD_MAP.get(category, [])
        for pat in patterns:
            combined_mask = combined_mask | sa_col.str.contains(re.escape(pat), na=False, regex=True)
...
def clean_sam_results(df, user_naics=None, user_states=None, user_keywords="", user_setasides=None):
    if df is None or df.empty:
        return df
    df = df.copy()
    rename_map = {
    ...
    if "Posted Date" in df.columns:
        ...
        df["Posted Date Sort"] = posted_sort
        df["Posted Date"] = posted_sort.dt.strftime("%Y-%m-%d").fillna("—")

    if "Response Deadline" in df.columns:
        response_deadline_sort = parse_datetime_series_mixed(df["Response Deadline"])
        df["Response Deadline Sort"] = response_deadline_sort
        df["Response Deadline"] = response_deadline_sort.dt.strftime("%Y-%m-%d").fillna("—")

    # ---- Set-Aside filter ----
    if user_setasides:
        df = filter_by_setaside(df, user_setasides)
        if df.empty:
            return df.reset_index(drop=True)
...
# EXPORT HELPERS
# ================================================================
def df_to_csv_bytes(df: pd.DataFrame, source_name: str) -> bytes:
    export = table_view_df(df, source_name)
    return export.to_csv(index=False).encode("utf-8")

def dfs_to_excel_bytes(sam_df: pd.DataFrame, usa_df: pd.DataFrame) -> bytes:
    buffer = io.BytesIO()
    try:
        with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
            if not sam_df.empty:
                table_view_df(sam_df, "SAM.gov").to_excel(writer, sheet_name="SAM.gov", index=False)
            if not usa_df.empty:
                table_view_df(usa_df, "USAspending").to_excel(writer, sheet_name="USAspending", index=False)
    except Exception:
        return None
    buffer.seek(0)
    return buffer.read()
...
def render_saved_opportunities():
    ...
    for opp in saved:
        ...
        score_badge = (
            f'<span class="saved-opp-badge {pill_cls}" style="padding:0.2rem 0.5rem; border-radius:999px; font-size:0.72rem; font-weight:700;">'
            f'{fit_score} · {fit_label}</span>'
...
# ---- Export Panel ----
def render_export_panel(clean_sam_df, clean_usa_df):
    with st.expander("📁 Export Results to CSV / Excel"):
        st.caption("Download your current search results for offline analysis.")
        ec1, ec2, ec3, ec4 = st.columns(4)

        with ec1:
            if not clean_sam_df.empty:
                csv_bytes = df_to_csv_bytes(clean_sam_df, "SAM.gov")
                st.download_button(
                    label="📥 SAM.gov CSV",
                    data=csv_bytes,
                    file_name=f"sam_opportunities_{date.today()}.csv",
                    mime="text/csv",
                    use_container_width=True,
                )
            else:
                st.button("📥 SAM.gov CSV", disabled=True, use_container_width=True)

        with ec2:
            if not clean_usa_df.empty:
                csv_bytes = df_to_csv_bytes(clean_usa_df, "USAspending")
                st.download_button(
                    label="📥 USAspending CSV",
                    data=csv_bytes,
                    file_name=f"usaspending_awards_{date.today()}.csv",
                    mime="text/csv",
                    use_container_width=True,
                )
            else:
                st.button("📥 USAspending CSV", disabled=True, use_container_width=True)

        with ec3:
            if not clean_sam_df.empty or not clean_usa_df.empty:
                parts = []
                if not clean_sam_df.empty:
                    parts.append(table_view_df(clean_sam_df, "SAM.gov"))
                if not clean_usa_df.empty:
                    parts.append(table_view_df(clean_usa_df, "USAspending"))
                combined = pd.concat(parts, ignore_index=True)
                combined_csv = combined.to_csv(index=False).encode("utf-8")
                st.download_button(
                    label="📥 Combined CSV",
                    data=combined_csv,
                    file_name=f"combined_results_{date.today()}.csv",
                    mime="text/csv",
                    use_container_width=True,
                )
            else:
                st.button("📥 Combined CSV", disabled=True, use_container_width=True)

        with ec4:
            excel_bytes = dfs_to_excel_bytes(clean_sam_df, clean_usa_df)
            if excel_bytes:
                st.download_button(
                    label="📊 Export Excel",
                    data=excel_bytes,
                    file_name=f"contract_results_{date.today()}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True,
                )
            else:
                st.caption("Excel export requires `openpyxl`. Install it to enable.")
...
min_value  = st.sidebar.number_input(
    "Min Value ($)", min_value=0, max_value=1_000_000_000, value=0, step=1000,
    key="widget_min_value",
)
max_value  = st.sidebar.number_input(
    "Max Value ($)", min_value=0, max_value=1_000_000_000, value=0, step=1000,
    key="widget_max_value",
...
# ---- Set-Aside Filter ----
st.sidebar.markdown("**Set-Aside Filter** (SAM.gov)")
selected_setasides = st.sidebar.multiselect(
    "Set-Aside Type(s)",
    options=SET_ASIDE_OPTIONS,
    default=[],
    key="widget_setasides",
    help="Filter SAM.gov results to only show opportunities with matching set-aside designations.",
)
...
def fetch_usaspending(sel_naics, s_year, e_year, min_val, max_val, kws, status_placeholder=None):
    """
    ...
    or MAX_PAGES is reached. Applies min and max value filters.
    """
    all_rows = []
    page     = 1
...
if search_clicked:
        ...
        with st.spinner("Fetching all USAspending award records..."):
            try:
                us_df = fetch_usaspending(
                    selected_naics, start_year, end_year, min_value, max_value, keywords,
    ...
    clean_sam_df = clean_sam_results(
        sam_df, selected_naics, states, keywords, selected_setasides
    ) if not sam_df.empty else pd.DataFrame()
...
if st.session_state.search_done:
    ...
    # ---- Export Results ----
    render_export_panel(clean_sam_df, clean_usa_df)
